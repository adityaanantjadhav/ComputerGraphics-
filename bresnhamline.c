...
void mouse(int button, int state, int x, int y) {           //mouse function take input with mouse 
    y = 480-y;
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {     //press the left click for first vertex input
       xi=x;
       yi=y;
    }
    if(button == GLUT_LEFT_BUTTON && state == GLUT_UP){         //leave the left click for second vertex input
    	xj=x;
        yj=y;
