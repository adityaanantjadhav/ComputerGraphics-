...
void floodfill(float* back, float* fill,int x,int y){
    float color[3];
	glReadPixels(x,y,1,1,GL_RGB,GL_FLOAT,color);
	if((color[0]==back[0] && color[1]==back[1] &&color[2]==back[2])&&(color[0]!=fill[0] ||color[1]!=fill[1] ||color[2]!=fill[2])){
		glBegin(GL_POINTS);
		glColor3f(fill[0],fill[1],fill[2]);
		glVertex2d(x,y);
		glEnd();
		glFlush();
		floodfill(back,fill,x+2,y);
		floodfill(back,fill,x-2,y);
		floodfill(back,fill,x,y+2);
		floodfill(back,fill,x,y-2);	
	}
}
void mouse(int button,int state,int x, int y){
	...
	if(button==GLUT_LEFT_BUTTON && state == GLUT_DOWN){
		xinside=x;
		yinside=y;
		floodfill(back,fill,xinside,yinside);
	}
}
void draw(){
